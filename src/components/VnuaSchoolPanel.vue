...
<template>
  <section class="card vnua-card">
    ...
    <div v-if="open" class="vnua-content">
      <p class="vnua-note">
        Token được giữ theo phiên và tự đăng nhập lại khi API phụ trả về 401/403/500. Hãy chạy bằng npm run dev hoặc npm run start.
      </p>
      <p class="vnua-note" v-if="api.activeBaseUrl">
        API đang dùng: {{ api.activeBaseUrl }}
      </p>
      ...
      <div class="toolbar" v-else>
        ...
        <div class="actions">
          <button class="secondary-btn" type="button" :disabled="api.loading" @click="toggleLoginForm">
            {{ showLoginForm ? 'Ẩn form đăng nhập' : 'Đổi tài khoản' }}
          </button>
          ...
          <button class="secondary-btn" type="button" :disabled="api.loading" @click="() => { api.logoutStudent(); showLoginForm.value = true; }">
            <LogOut :size="16" />
            Đăng xuất
          </button>
        </div>
